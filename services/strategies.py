...
def connors_rsi2_trades(ohlc: pd.DataFrame, signals: pd.DataFrame, max_lookback: int | None = None) -> list[dict]:
    """Generate trade records for Larry Connors RSI(2) Mean Reversion strategy."""
    if not ohlc.index.equals(signals.index):
        raise ValueError("OHLC and signal indexes must match")
    n = len(ohlc)
    start_pos = max(0, n - max_lookback) if max_lookback is not None else 0
    ...
    for position in range(start_pos, n):
        ...
        sig = signals.iloc[position]
        signal_date = ohlc.index[position].date().isoformat()
        rsi2_val = round(float(sig["rsi2"]), 2) if "rsi2" in sig and pd.notna(sig["rsi2"]) else None

        if bool(sig.get("buy_signal", False)):
            rows.append({
                ...
                "confirmation_date": signal_date,
                "entry_price": float(signal_row["Close"]),
                "stop_loss": round(float(signal_row["Close"] * 0.95), 2),
                "rsi_value": rsi2_val,
            })
        elif bool(sig.get("sell_signal", False)):
            rows.append({
                "strategy": "CONNORS_RSI2",
                "signal_type": "sell",
                "signal_date": signal_date,
                ...
                "confirmation_date": signal_date,
                "entry_price": float(signal_row["Close"]),
                "stop_loss": round(float(signal_row["Close"] * 1.05), 2),
                "rsi_value": rsi2_val,
            })
    return rows
